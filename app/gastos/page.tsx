...
import { GastosSkeleton } from "@/components/panel/loading-skeletons";
...
function dbErrorMessage(error: { code?: string; message?: string } | null, fallback: string): string {
  ...
  if (error.code === "42P01") {
    return "No existen las tablas de gastos/categorias. Ejecuta supabase/expenses.sql en Supabase.";
  }
...
function getCategoryMain(category?: ExpenseCategory | null): string {
  if (!category) return "-";
  const main = category.category_name?.trim();
  if (main) return main;
...
function getCategorySubcategory(category?: ExpenseCategory | null): string {
  if (!category) return "-";
  const sub = category.subcategory_name?.trim();
  if (sub) return sub;
...
export default function GastosPage() {
  ...
  const [subcategoryFilterMode, setSubcategoryFilterMode] = useState<"include" | "exclude">("include");
  const [selectedSubcategories, setSelectedSubcategories] = useState<string[]>([]);
  const [accountFilterMode, setAccountFilterMode] = useState<"include" | "exclude">("include");
  const [selectedBankAccountIds, setSelectedBankAccountIds] = useState<string[]>([]);
  const [userFilterMode, setUserFilterMode] = useState<"include" | "exclude">("include");
  const [selectedUserIds, setSelectedUserIds] = useState<string[]>([]);
  const [openLogFilter, setOpenLogFilter] = useState<
    "date" | "category" | "subcategory" | "account" | "user" | null
  >(null);
  ...
  const [draftSubcategoryMode, setDraftSubcategoryMode] = useState<"include" | "exclude">("include");
  const [draftSubcategories, setDraftSubcategories] = useState<string[]>([]);
  const [draftAccountMode, setDraftAccountMode] = useState<"include" | "exclude">("include");
  const [draftBankAccountIds, setDraftBankAccountIds] = useState<string[]>([]);
  const [draftUserMode, setDraftUserMode] = useState<"include" | "exclude">("include");
  const [draftUserIds, setDraftUserIds] = useState<string[]>([]);
  const [userFilterOptions, setUserFilterOptions] = useState<UserFilterOption[]>([]);
  ...
  const subcategoryOptions = useMemo(() => {
    ...
    for (const category of categories) {
      const label = getCategorySubcategory(category);
      if (label && label !== "-") unique.add(label);
    }
    return [...unique].sort((a, b) => a.localeCompare(b, "es"));
  }, [categories]);
  const selectedSubcategoryCategoryIds = useMemo(() => {
    if (selectedSubcategories.length === 0) return [] as string[];
    return categories
      .filter((category) => selectedSubcategories.includes(getCategorySubcategory(category)))
  ...
  const loadUserFilterOptions = useCallback(async () => {
    ...
    if (selectedSubcategories.length > 0 && selectedSubcategoryCategoryIds.length === 0) {
      setUserFilterOptions([]);
      return;
    }
    ...
    if (selectedSubcategories.length > 0) {
      if (subcategoryFilterMode === "include") {
        query = query.in("category_id", selectedSubcategoryCategoryIds);
      } else {
        for (const categoryId of selectedSubcategoryCategoryIds) {
          query = query.neq("category_id", categoryId);
        }
      }
    }
    ...
    if (selectedBankAccountIds.length > 0) {
      if (accountFilterMode === "include") {
        query = query.in("bank_account_id", selectedBankAccountIds);
      } else {
        for (const accountId of selectedBankAccountIds) {
          query = query.neq("bank_account_id", accountId);
        }
      }
    }
    ...
    for (const row of data ?? []) {
      const userId = row.registered_by;
      if (!userId) continue;
      const label = row.registered_by_name || row.registered_by_email || userId;
      if (!optionMap.has(userId)) {
        optionMap.set(userId, { id: userId, label });
      }
    }

    setUserFilterOptions(
      [...optionMap.values()].sort((a, b) => a.label.localeCompare(b.label, "es"))
    );
  }, [
    accountFilterMode,
    categoryFilterMode,
    ...
    selectedSubcategoryCategoryIds,
  ...
  const loadSummary = useCallback(async () => {
    ...
    if (error) {
      setSummaryError(dbErrorMessage(error, "No se pudo cargar resumen de gastos."));
      return;
    }
  ...
  const loadLogs = useCallback(async () => {
    ...
    if (selectedSubcategories.length > 0) {
      if (selectedSubcategoryCategoryIds.length === 0) {
        setLogs([]);
        setLogsCount(0);
    ...
    if (selectedUserIds.length > 0) {
      if (userFilterMode === "include") {
        query = query.in("registered_by", selectedUserIds);
      } else {
        for (const userId of selectedUserIds) {
          query = query.neq("registered_by", userId);
        }
      }
    }
    ...
    if (error) {
      setLogsError(dbErrorMessage(error, "No se pudo cargar el registro de gastos."));
      return;
    }
  ...
  const onSubmitExpense = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const selectedBankAccountId = expenseBankAccountId || bankAccounts[0]?.id || "";
    ...
    if (!expenseDate) {
      setExpenseActionError("Selecciona la fecha del gasto.");
      return;
    }
    ...
    if (!selectedBankAccountId) {
      setExpenseActionError("Selecciona la cuenta bancaria.");
      return;
    }
    ...
    if (error) {
      setExpenseActionError(dbErrorMessage(error, "No se pudo registrar el gasto."));
      return;
    }

    setExpenseActionSuccess("Gasto registrado y saldo actualizado.");
    setExpenseModalOpen(false);
    setExpenseAmount("");
    setExpenseDescription("");
    setExpenseDate(toYmd(new Date()));
    await Promise.all([loadLogs(), loadSummary(), loadBankAccounts()]);
  };
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [loading, profile, loadUserFilterOptions]);
  ...
  const openSubcategoryFilter = () => {
    setDraftSubcategoryMode(subcategoryFilterMode);
    setDraftSubcategories(selectedSubcategories);
    setOpenLogFilter((current) => (current === "subcategory" ? null : "subcategory"));
  };

  const openAccountFilter = () => {
    setDraftAccountMode(accountFilterMode);
    setDraftBankAccountIds(selectedBankAccountIds);
    setOpenLogFilter((current) => (current === "account" ? null : "account"));
  };

  const openUserFilter = () => {
    setDraftUserMode(userFilterMode);
    setDraftUserIds(selectedUserIds);
    setOpenLogFilter((current) => (current === "user" ? null : "user"));
  };

  if (loading || !profile) return <GastosSkeleton />;

  return (
    <PanelShell
      title="Gastos"
      subtitle="Control de egresos, compras y pagos en PEN (S/.)"
      profile={profile}
      actions={
    ...
    >
      {(summaryError || categoriesError || bankAccountsError || logsError || categoryActionError || expenseActionError) && (
      ...
      <section className="grid gap-4 lg:grid-cols-2 xl:grid-cols-[1fr_1fr_1.2fr]">
        <article className="rounded-2xl border border-[#d7b7a0]/45 bg-white p-5 shadow-[0_6px_16px_rgba(10,25,59,0.12)]">
          <p className="text-sm text-[#0a193b]/70">Gastos totales del periodo</p>
          {summaryLoading ? (
        ...
        <article className="rounded-2xl border border-[#d7b7a0]/45 bg-white p-5 shadow-[0_6px_16px_rgba(10,25,59,0.12)]">
          <p className="text-sm text-[#0a193b]/70">Top 3 categorias con mas gastos</p>
          {summaryLoading ? (
        ...
        <article className="rounded-2xl border border-[#d7b7a0]/45 bg-white p-5 shadow-[0_6px_16px_rgba(10,25,59,0.12)]">
          <div className="flex items-start justify-between gap-2">
            ...
            <div className="flex flex-col items-end gap-2">
              <button
                type="button"
                disabled={categories.length === 0 || bankAccounts.length === 0}
                onClick={() => {
                  if (!expenseCategoryId && categories.length > 0) {
                    setExpenseCategoryId(categories[0].id);
                  }
                  if (!expenseBankAccountId && bankAccounts.length > 0) {
                    setExpenseBankAccountId(bankAccounts[0].id);
                  }
      ...
      <section className="rounded-2xl border border-[#d7b7a0]/45 bg-white p-5 shadow-[0_6px_16px_rgba(10,25,59,0.12)] sm:p-6">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-[#0a193b]">Registro de gastos</h2>
            <p className="mt-1 text-xs text-[#0a193b]/65">
              Filtros por columna: Fecha, Categoria, Subcategoria, Cuenta y Usuario.
            </p>
          </div>
        </div>
              ...
              <tr>
                ...
                <th className="relative px-3 py-3 font-semibold">
                  <div className="inline-flex items-center gap-1">
                    ...
                    <button
                      type="button"
                      onClick={openSubcategoryFilter}
                      className="rounded p-0.5 hover:bg-[#e8dfd8]"
                      title="Filtrar subcategoria"
                      aria-label="Filtrar subcategoria"
                    >
                      <FilterIcon active={hasSubcategoryFilter} />
                    </button>
                  </div>
                  {openLogFilter === "subcategory" && (
                    <div className="absolute left-2 top-9 z-30 w-72 rounded-lg border border-[#d7b7a0]/70 bg-white p-3 shadow-[0_12px_30px_rgba(10,25,59,0.2)]">
                      <p className="text-xs font-semibold text-[#0a193b]">Filtrar subcategoria</p>
                      <div className="mt-2 flex items-center gap-3">
                        <label className="flex items-center gap-1 text-[11px] text-[#0a193b]/85">
                          <input
                            type="radio"
                            checked={draftSubcategoryMode === "include"}
                            onChange={() => setDraftSubcategoryMode("include")}
                          />
                          Incluir
                        </label>
                        <label className="flex items-center gap-1 text-[11px] text-[#0a193b]/85">
                          <input
                            type="radio"
                            checked={draftSubcategoryMode === "exclude"}
                            onChange={() => setDraftSubcategoryMode("exclude")}
                          />
                          Excluir
                        </label>
                      </div>
                      <div className="mt-2 max-h-44 space-y-1 overflow-y-auto rounded-md border border-[#d7b7a0]/40 p-2">
                        {subcategoryOptions.map((subcategory) => {
                          const checked = draftSubcategories.includes(subcategory);
                          return (
                            <label
                              key={subcategory}
                              className="flex items-center gap-2 rounded px-1 py-1 text-xs text-[#0a193b]"
                        ...
                        {subcategoryOptions.length === 0 && (
                          <p className="text-xs text-[#0a193b]/65">No hay subcategorias.</p>
                        )}
                      </div>
                      <div className="mt-3 flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            setSubcategoryFilterMode(draftSubcategoryMode);
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setDraftSubcategoryMode("include");
                            setDraftSubcategories([]);
                            setSubcategoryFilterMode("include");
                ...
                <th className="relative px-3 py-3 font-semibold">
                  <div className="inline-flex items-center gap-1">
                    ...
                    <button
                      type="button"
                      onClick={openAccountFilter}
                      className="rounded p-0.5 hover:bg-[#e8dfd8]"
                      title="Filtrar cuenta"
                      aria-label="Filtrar cuenta"
                    >
                      <FilterIcon active={hasAccountFilter} />
                    </button>
                  </div>
                  {openLogFilter === "account" && (
                    <div className="absolute left-2 top-9 z-30 w-72 rounded-lg border border-[#d7b7a0]/70 bg-white p-3 shadow-[0_12px_30px_rgba(10,25,59,0.2)]">
                      <p className="text-xs font-semibold text-[#0a193b]">Filtrar cuenta</p>
                      <div className="mt-2 flex items-center gap-3">
                        <label className="flex items-center gap-1 text-[11px] text-[#0a193b]/85">
                          <input
                            type="radio"
                            checked={draftAccountMode === "include"}
                            onChange={() => setDraftAccountMode("include")}
                          />
                          Incluir
                        </label>
                        <label className="flex items-center gap-1 text-[11px] text-[#0a193b]/85">
                          <input
                            type="radio"
                            checked={draftAccountMode === "exclude"}
                            onChange={() => setDraftAccountMode("exclude")}
                          />
                          Excluir
                        </label>
                      </div>
                      <div className="mt-2 max-h-44 space-y-1 overflow-y-auto rounded-md border border-[#d7b7a0]/40 p-2">
                        {bankAccounts.map((account) => {
                          const checked = draftBankAccountIds.includes(account.id);
                          return (
                            <label
                              key={account.id}
                              className="flex items-center gap-2 rounded px-1 py-1 text-xs text-[#0a193b]"
                            >
                              <input
                                ...
                                checked={checked}
                                onChange={() => {
                                  setDraftBankAccountIds((current) => {
                      ...
                      <div className="mt-3 flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            setAccountFilterMode(draftAccountMode);
                            setSelectedBankAccountIds(draftBankAccountIds);
                            setLogsPage(1);
                            setOpenLogFilter(null);
                          }}
                          className="rounded-md bg-[#0a193b] px-2.5 py-1.5 text-xs font-semibold text-white"
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setDraftAccountMode("include");
                            setDraftBankAccountIds([]);
                            setAccountFilterMode("include");
                ...
                <th className="relative px-3 py-3 font-semibold">
                  <div className="inline-flex items-center gap-1">
                    ...
                    >
                      <FilterIcon active={hasUserFilter} />
                    </button>
                  </div>
                  {openLogFilter === "user" && (
                    <div className="absolute right-2 top-9 z-30 w-72 rounded-lg border border-[#d7b7a0]/70 bg-white p-3 shadow-[0_12px_30px_rgba(10,25,59,0.2)]">
                      <p className="text-xs font-semibold text-[#0a193b]">Filtrar usuario</p>
                      <div className="mt-2 flex items-center gap-3">
                        <label className="flex items-center gap-1 text-[11px] text-[#0a193b]/85">
                          <input
                            type="radio"
                            checked={draftUserMode === "include"}
                            onChange={() => setDraftUserMode("include")}
                          />
                          Incluir
                        </label>
                        <label className="flex items-center gap-1 text-[11px] text-[#0a193b]/85">
                          <input
                            type="radio"
                            checked={draftUserMode === "exclude"}
                            onChange={() => setDraftUserMode("exclude")}
                          />
                          Excluir
                        </label>
                      </div>
                      <div className="mt-2 max-h-44 space-y-1 overflow-y-auto rounded-md border border-[#d7b7a0]/40 p-2">
                        ...
                        {userFilterOptions.length === 0 && (
                          <p className="text-xs text-[#0a193b]/65">No hay usuarios en este rango.</p>
                        )}
                      </div>
                      <div className="mt-3 flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            setUserFilterMode(draftUserMode);
                        ...
                        <button
                          type="button"
                          onClick={() => {
                            setDraftUserMode("include");
                            setDraftUserIds([]);
                            setUserFilterMode("include");
                            setSelectedUserIds([]);
                            setLogsPage(1);
                            setOpenLogFilter(null);
                          }}
                ...
                : visibleLogRows.map((row, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-3">
                        {row
                          ? row.category_id
                            ? getCategoryMain(categoryMap.get(row.category_id))
                            : "-"
                          : ""}
                      </td>
                      <td className="px-3 py-3">
                        {row
                          ? row.category_id
                            ? getCategorySubcategory(categoryMap.get(row.category_id))
                            : "-"
                          : ""}
                      </td>
                      <td className="px-3 py-3">
                        {row
                          ? row.bank_account_id
                            ? bankAccountMap.get(row.bank_account_id)?.name || "Cuenta eliminada"
                            : "-"
                          : ""}
                      </td>
                      <td className="px-3 py-3">{row ? row.description || "-" : ""}</td>
                      <td className="px-3 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4">
          <section className="w-full max-w-lg rounded-2xl border border-[#d7b7a0]/55 bg-white p-5 shadow-[0_20px_45px_rgba(10,25,59,0.3)] sm:p-6">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-2xl font-semibold text-[#0a193b]">Registrar gasto</h2>
                <p className="mt-1 text-sm text-[#0a193b]/70">
                  El gasto descuenta saldo de la cuenta bancaria seleccionada.
                </p>
              </div>
              <button
                type="button"
            ...
            <form className="space-y-4" onSubmit={onSubmitExpense}>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-[#0a193b]" htmlFor="expenseBankAccount">
                  Cuenta bancaria
                </label>
                <select
                  id="expenseBankAccount"
                  value={expenseBankAccountId || bankAccounts[0]?.id || ""}
                ...
                >
                  {bankAccounts.map((account) => (
                    <option key={account.id} value={account.id}>
                      {account.name} - saldo {formatPen(Number(account.current_balance) || 0)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <button
                  type="submit"
                  disabled={savingExpense || categories.length === 0 || bankAccounts.length === 0}
